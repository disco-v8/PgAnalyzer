...
int config_str_convert(char *target, int target_len)
{
	...
	char                            value[10][MAX_STRING_LENGTH];       // 各設定値文字列
	...
	else if (strcmp("LOGFILE", key_str) == 0)
	{
		...
		// ログファイルが開いていたら(この時点ではスタートアップログファイルが開いているはず)
		if (EVS_log_fd != 0)
		{
			// 既存のログファイルへバッファをフラッシュする
			fsync(EVS_log_fd);
			// 既存のログファイルを閉じる
			EVS_log_fd = close(EVS_log_fd);
	...
	// PostgreSQL設定なら
	// ----------------
	else if (strcmp("POSTGRESQL", key_str) == 0)
	{
		// データベース別設定用構造体ポインタのメモリ領域を確保
		db_list = (struct EVS_db_t *)calloc(1, sizeof(struct EVS_db_t));
		// メモリ領域が確保できなかったら
		if (db_list == NULL)
		{
			snprintf(log_str, MAX_LOG_LENGTH, "%s(): Cannot calloc memory? errno=%d (%s)\n", __func__, errno, strerror(errno));
		...
		// 設定値を個別に変換、その2 ※パラメータが取得できた数がinit_resultに設定される
		init_result = sscanf(value_str, "%[^,],%[^,],%[^,],%[^,],%[^,]", value[0], value[1], value[2], value[3], value[4]);
		// 変換数が3未満なら
		if (init_result < 3)
		{
			snprintf(log_str, MAX_LOG_LENGTH, "%s(): Not enough PostgreSQL's parameters!? %d %s\n", __func__, init_result, value_str);
			logging(LOG_DIRECT, LOGLEVEL_ERROR, NULL, NULL, NULL, log_str, strlen(log_str));
			// 設定値名のメモリ領域は不要になったので破棄
			free(key_str);
			// 設定値のメモリ領域は不要になったので破棄
		...
		// データベース名を設定
		strncpy(db_list->database, value[0], 63);
		// ユーザー名を設定
		strncpy(db_list->username, value[1], 31);
		// パスワードを設定
		strncpy(db_list->password, value[2], 31);
		// 変換数が3なら
		if (init_result == 3)
		{
			// ホスト名として"UNIXSOCKET"を設定
			strncpy(db_list->hostname, "UNIXSOCKET", 127);
			// ポートとして0を設定
			db_list->port = 0;
		}
		// 変換数が4なら
		...
		{
			// ホスト名を設定
			strncpy(db_list->hostname, value[3], 127);
			// サービス名を設定
			strncpy(db_list->servicename, "postgres", 15);
			// ポートとして5432を設定
			db_list->port = 5432;
		}
		// 変換数が5以上なら
		...
		{
			// ホスト名を設定
			strncpy(db_list->hostname, value[3], 128);
			// サービス名を設定
			strncpy(db_list->servicename, value[4], 15);
			// 設定値が数値変換できて、かつ1～65535なら
			if (atoi(value[4]) >= 1 && atoi(value[4]) <=65535)
			{
				db_list->port = atoi(value[4]);                                 // ポート番号を設定する(実際にはservicenameを使うのでポート番号は使わない)
			}
			else
			{
				db_list->port = 5432;                                           // ポート番号を設定する(実際にはservicenameを使うのでポート番号は使わない)
			}
		}
		snprintf(log_str, MAX_LOG_LENGTH, "%s(): PostgreSQL's database=%s, username=%s, password=%s, hostname=%s, port=%d\n", __func__, db_list->database, db_list->username, db_list->password, db_list->hostname, db_list->port); 
		logging(LOG_QUEUEING, LOGLEVEL_INFO, NULL, NULL, NULL, log_str, strlen(log_str));
		// テールキューの最後にデータベースの情報を追加する
		TAILQ_INSERT_TAIL(&EVS_db_tailq, db_list, entries);
		snprintf(log_str, MAX_LOG_LENGTH, "%s(): TAILQ_INSERT_TAIL(db_list): OK.\n", __func__);
		logging(LOG_QUEUEING, LOGLEVEL_INFO, NULL, NULL, NULL, log_str, strlen(log_str));
	}
	else
	{
...
int INIT_config_default(void)
{
	...
	// ログファイルを設定
	// ----------------
	char                            *log_file = "/var/log/EvServer/Startup.log";
	// 設定値文字列のメモリ領域を確保(+ 1バイトを忘れずに!!)
